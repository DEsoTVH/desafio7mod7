...
const cafes = require("./cafes.json"); // Modifica la ruta si es necesario
...
app.post("/cafes", (req, res) => {
  ...
  if (existeUnCafeConEseId) {
    res.status(400).send({ message: "Ya existe un café con ese id" });
  } else {
    cafes.push(cafe);
...
app.put("/cafes/:id", (req, res) => {
  ...
  if (id !== cafeActualizado.id) {
    res.status(400).send({ message: "El id del parámetro no coincide con el id del café recibido" });
  } else {
    const cafeIndexFound = cafes.findIndex(c => c.id == id);
    if (cafeIndexFound >= 0) {
      cafes[cafeIndexFound] = cafeActualizado;
      res.send(cafes);
    } else {
      res.status(404).send({ message: "No se encontró ningún café con ese id" });
    }
